...
export async function convertUrlsWithRateLimit(
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    // Progress indicator
    if ((i + 1) % 10 === 0) {
      process.stdout.write(`\r   Converting: ${i + 1}/${urls.length} URLs`)
    }
  }
...
// Test if run directly (only when called with a URL argument, not when imported)
const testUrl = process.argv[2]
if (testUrl && (testUrl.includes('photos.app.goo.gl') || testUrl.includes('photos.google.com'))) {
  console.log(`Testing URL: ${testUrl}`)
  getDirectPhotoUrl(testUrl).then((result) => {
    if (result) {
