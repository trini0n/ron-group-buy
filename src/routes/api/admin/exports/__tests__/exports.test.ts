...
describe('Export File Content Validation', () => {
  it('should generate valid Excel file structure', async () => {
    const { exportSingleOrder: realExportSingleOrder } =
      await vi.importActual<typeof import('$lib/server/export-builder')>('$lib/server/export-builder')
  ...
  it('should handle multi-tab export with correct tab ordering', async () => {
    const { exportGroupBuyOrders: realExportGroupBuyOrders } =
      await vi.importActual<typeof import('$lib/server/export-builder')>('$lib/server/export-builder')

    const makeOrder = (id: string, orderNumber: string, shippingType: string, createdAt: string) => ({
      id,
