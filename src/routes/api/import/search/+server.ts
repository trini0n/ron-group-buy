...
async function searchSingleCard(supabase: SupabaseClient, card: DeckCard): Promise<SearchResult> {
  ...
  if (!allMatches) {
    ...
    const allResults = [...(exactMatches || []), ...(flavorMatches || []), ...(doubleFacedMatches || [])]
  ...
  if (card.set && card.collectorNumber) {
    exactMatch =
      sorted.find(
        (m) => m.set_code?.toLowerCase() === card.set?.toLowerCase() && m.collector_number === card.collectorNumber
      ) ?? null
  }

  // Fallback: match by set only
  if (!exactMatch && card.set) {
    exactMatch = sorted.find((m) => m.set_code?.toLowerCase() === card.set?.toLowerCase()) ?? null
  }
...
async function parallelSearchCards(supabase: SupabaseClient, cards: DeckCard[]): Promise<SearchResult[]> {
  ...
  const uniqueResults = await Promise.all(uniqueCardsArray.map((card) => searchSingleCard(supabase, card)))
