<script lang="ts">
  ...
  function getCardImageUrl(card: InventoryCard): string {
    ...
    return card.scryfall_id ? getScryfallImageUrl(card.scryfall_id, 'normal') : '/images/card-placeholder.png'
  }
</script>
...
<div class="p-8">
          ...
          <Command.List>
            ...
            <Command.Group>
              {#each filteredSets as set (set.code)}
                <Command.Item value={set.code} onSelect={() => toggleSet(set.code)}>
                  <Check class="mr-2 h-4 w-4 {selectedSets.includes(set.code) ? 'opacity-100' : 'opacity-0'}" />
  ...
  {#if selectedCards.size > 0}
    <div class="mb-4 flex items-center gap-4 rounded-lg border bg-muted/50 p-4">
      ...
      <Button size="sm" variant="outline" onclick={() => updateStock([...selectedCards], true)} disabled={isUpdating}>
        <Package class="mr-2 h-4 w-4" />
        Mark In Stock
      </Button>
      <Button size="sm" variant="outline" onclick={() => updateStock([...selectedCards], false)} disabled={isUpdating}>
      ...
      <Button size="sm" variant="ghost" onclick={() => (selectedCards = new Set())}>Clear Selection</Button>
    </div>
  {/if}
    ...
    <Table.Root>
      ...
      <Table.Body>
        {#each filteredCards as card}
          <Table.Row
            class="{selectedCards.has(card.id) ? 'bg-muted/50' : ''} {card.isDuplicate
              ? 'border-l-4 border-l-orange-500'
              : ''}"
            onmouseleave={handleRowMouseLeave}
          >
            <Table.Cell>
              <Checkbox checked={selectedCards.has(card.id)} onCheckedChange={() => toggleCardSelection(card.id)} />
            </Table.Cell>
            <Table.Cell class="font-medium" onmousemove={(e) => handleRowMouseMove(e, card)}>
              {card.card_name}
              {#if card.is_new}
            ...
            <Table.Cell class="font-mono text-sm" onmousemove={(e) => handleRowMouseMove(e, card)}>
              {card.serial}
            </Table.Cell>
            <Table.Cell class="text-sm" onmousemove={(e) => handleRowMouseMove(e, card)}>
              {card.set_name || card.set_code}
            </Table.Cell>
            <Table.Cell class="text-xs font-mono uppercase" onmousemove={(e) => handleRowMouseMove(e, card)}>
              {card.set_code || '—'}
            </Table.Cell>
            <Table.Cell class="text-sm" onmousemove={(e) => handleRowMouseMove(e, card)}>
            ...
            <Table.Cell onmousemove={(e) => handleRowMouseMove(e, card)}>
              {#if card.is_in_stock}
                <Badge class="bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300">In Stock</Badge>
              {:else}
        ...
        {:else}
          <Table.Row>
            <Table.Cell colspan={10} class="py-8 text-center text-muted-foreground">No cards found</Table.Cell>
          </Table.Row>
        {/each}
      </Table.Body>
    </Table.Root>
  ...
  {#if totalPages > 1}
    <div class="mt-4 flex items-center justify-between">
      ...
      <div class="flex gap-2">
        <Button variant="outline" size="sm" disabled={data.page <= 1} onclick={() => changePage(data.page - 1)}>
