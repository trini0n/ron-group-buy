<script lang="ts">
  ...
  // Auth management
  const authMethods = $derived(data.authMethods);
  const authMethodCount = $derived(
    (authMethods.hasGoogle ? 1 : 0) +
    (authMethods.hasDiscord ? 1 : 0) +
    (authMethods.hasPassword ? 1 : 0) +
    // Also count from users table as fallback for deduplication
    ((!authMethods.hasGoogle && (data.user as any).google_id) ? 1 : 0) +
    ((!authMethods.hasDiscord && (data.user as any).discord_id) ? 1 : 0)
  );
  let isUnlinkDialogOpen = $state(false);
  let unlinkingProvider = $state<'google' | 'discord' | null>(null);
  let isUnlinking = $state(false);

  function openUnlinkDialog(provider: 'google' | 'discord') {
    if (authMethodCount <= 1) {
      toast.error('Cannot remove last authentication method — user would be locked out');
      return;
    }
  ...
  async function confirmUnlink() {
    ...
    const provider = unlinkingProvider;
    const providerName = provider === 'google' ? 'Google' : 'Discord';
    isUnlinking = true;

    try {
      const response = await fetch(`/api/admin/users/${user.id}/unlink-provider`, {
        method: 'POST',
      ...
      <Card.Root>
        ...
        <Card.Content class="space-y-3">
          ...
          <div class="flex items-center justify-between rounded-lg border p-3">
            ...
            <div class="flex items-center gap-2">
              {#if authMethods.hasGoogle || user.google_id}
                <Badge variant="outline" class="text-green-600">Linked</Badge>
                <Button
                  variant="ghost"
                  size="sm"
                  class="h-7 text-destructive hover:text-destructive"
                  onclick={() => openUnlinkDialog('google')}
                  disabled={authMethodCount <= 1}
                  title={authMethodCount <= 1 ? 'Cannot remove last auth method' : 'Unlink Google'}
                >
          ...
          <div class="flex items-center justify-between rounded-lg border p-3">
            <div class="flex items-center gap-2">
              ...
              {#if user.discord_username}
                <span class="text-xs text-muted-foreground">({user.discord_username})</span>
              {/if}
            </div>
            <div class="flex items-center gap-2">
              {#if authMethods.hasDiscord || user.discord_id}
                <Badge variant="outline" class="text-green-600">Linked</Badge>
                <Button
                  variant="ghost"
                  size="sm"
                  ...
                  disabled={authMethodCount <= 1}
                  title={authMethodCount <= 1 ? 'Cannot remove last auth method' : 'Unlink Discord'}
                >
                  <Unlink class="h-3.5 w-3.5" />
                </Button>
              {:else}
          ...
          {#if authMethodCount <= 1}
            <p class="text-xs text-muted-foreground mt-2">
              User has only one authentication method. Unlinking is disabled to prevent lockout.
            </p>
          {/if}
        </Card.Content>
      </Card.Root>
...
<Dialog.Root bind:open={isUnlinkDialogOpen}>
  <Dialog.Content class="sm:max-w-md">
    <Dialog.Header>
      <Dialog.Title>Unlink {unlinkingProvider === 'google' ? 'Google' : 'Discord'} Account</Dialog.Title>
      <Dialog.Description>
        This will remove the {unlinkingProvider === 'google' ? 'Google' : 'Discord'} OAuth connection from
        <strong>{user.name || user.email}</strong>'s account. They will no longer be able to sign in with this provider.
      </Dialog.Description>
    </Dialog.Header>
    <Dialog.Footer class="flex gap-2 sm:justify-end">
      <Button variant="outline" onclick={() => { isUnlinkDialogOpen = false; unlinkingProvider = null; }}>
        Cancel
      </Button>
