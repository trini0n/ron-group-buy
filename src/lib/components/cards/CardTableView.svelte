...
{:else}
  ...
  {#if selectedSerials.size > 0}
    <div class="mb-4 flex items-center gap-4 rounded-lg border bg-muted/50 p-3">
      ...
      <Button size="sm" variant="ghost" onclick={() => (selectedSerials = new Set())}>Clear Selection</Button>
    </div>
  {/if}
  ...
  <div class="mb-4 flex items-center justify-between text-sm text-muted-foreground">
    <span>
      Showing {(currentPage - 1) * CARDS_PER_PAGE + 1}–{Math.min(currentPage * CARDS_PER_PAGE, sortedCards.length)} of {sortedCards.length}
      cards
    </span>
    {#if totalPages > 1}
      ...
      <Table.Body>
        {#each paginatedCards as card (card.serial)}
          ...
          <Table.Row class={isSelected ? 'bg-muted/50' : ''}>
            ...
            <Table.Cell onmousemove={(e) => handleRowMouseMove(e, card)} onmouseleave={handleRowMouseLeave}>
